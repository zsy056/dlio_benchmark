...
def test_adls_checkpoint_duration_options_are_configurable():
    from dlio_benchmark.checkpointing.pytorch_adls_checkpointing import (
        DEFAULT_CONTAINER_SAS_REFRESH_MARGIN,
        DEFAULT_CONTAINER_SAS_TTL,
        PyTorchADLSCheckpointing,
    )

    checkpoint = PyTorchADLSCheckpointing.__new__(PyTorchADLSCheckpointing)

    assert checkpoint._get_duration_option({}, "container_sas_ttl", DEFAULT_CONTAINER_SAS_TTL) == DEFAULT_CONTAINER_SAS_TTL
    assert checkpoint._get_duration_option({"container_sas_ttl": 90}, "container_sas_ttl", DEFAULT_CONTAINER_SAS_TTL) == timedelta(seconds=90)
    assert checkpoint._get_duration_option({"container_sas_ttl": "2h"}, "container_sas_ttl", DEFAULT_CONTAINER_SAS_TTL) == timedelta(hours=2)
    assert checkpoint._get_duration_option({"sas_refresh_margin": "7.5m"}, "sas_refresh_margin", DEFAULT_CONTAINER_SAS_REFRESH_MARGIN) == timedelta(minutes=7.5)
